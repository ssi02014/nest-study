...
describe('PaymentsService (예외 검증)', () => {
  ...
  describe('processPayment', () => {
    it('주문이 존재하지 않으면 NotFoundException을 던진다', async () => {
      ...
      await expect(service.processPayment(999, 10000)).rejects.toThrow(
        NotFoundException
      );
      // 예외 메시지까지 정확히 검증
      await expect(service.processPayment(999, 10000)).rejects.toThrow(
    ...
    it('결제 금액이 0 이하면 BadRequestException을 던진다', async () => {
      ...
      await expect(service.processPayment(1, -100)).rejects.toThrow(
        BadRequestException
      );
      await expect(service.processPayment(1, -100)).rejects.toThrow(
        '결제 금액은 0보다 커야 합니다.'
    ...
    it('결제 게이트웨이 오류 시 ServiceUnavailableException을 던진다', async () => {
      ...
      await expect(service.processPayment(1, 10000)).rejects.toThrow(
        ServiceUnavailableException
      );
    });
...
describe('ItemsController (E2E)', () => {
  ...
  it('GET /items/999 → 존재하지 않는 아이템 조회 시 404를 반환한다', () => {
    return request(app.getHttpServer()).get('/items/999').expect(404);
  });
});
```
...
describe('PostsService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<PostsService>(PostsService);
    postsRepository = module.get<MockRepository<Post>>(
  ...
  describe('update', () => {
    ...
    it('존재하지 않는 게시글 수정 시 NotFoundException을 던진다', async () => {
      ...
      await expect(service.update(999, { title: '수정' })).rejects.toThrow(
        NotFoundException
      );
    });
  });
