...
일반적으로 Provider는 **애플리케이션 생명주기와 동기화된 스코프**를 가진다. 앱이 부트스트랩될 때 모든 의존성이 해결되며 Provider가 인스턴스화되고, 앱이 종료될 때 각 Provider도 함께 소멸된다.

이 기본 동작이 **싱글톤 스코프**다. 앱 전체에서 인스턴스가 하나만 생성되어 재사용된다. 필요에 따라 아래와 같이 스코프를 변경할 수 있다.
