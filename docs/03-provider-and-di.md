...
클래스를 Provider로 만들려면 `@Injectable()` 데코레이터를 붙인다. 이 데코레이터가 있어야 NestJS IoC 컨테이너가 해당 클래스를 인식하고 관리할 수 있다.
...
`@Injectable()`을 붙인 뒤에는 반드시 **모듈의 `providers` 배열에 등록**해야 한다.
...
## @Optional()과 forwardRef()

### @Optional() - 선택적 의존성

Provider가 등록되어 있지 않아도 에러가 나지 않게 하려면 `@Optional()` 데코레이터를 사용한다. 등록되지 않은 경우 `undefined`가 주입된다.
...
| 개념 | 핵심 |
|------|------|
| **Provider** | `@Injectable()`로 선언, 모듈의 `providers`에 등록하면 NestJS가 관리 |
