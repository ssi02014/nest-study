...
> **팁:** 컨트롤러 클래스로 지정하면 컨트롤러의 라우트 접두사(`@Controller('posts')`)가 변경되어도 `forRoutes()`를 수정할 필요가 없다. 문자열 경로보다 유지보수에 유리하다.

### 6-4. 특정 라우트 제외하기 (exclude)

`exclude()` 메서드로 특정 경로를 미들웨어 적용에서 제외할 수 있다. `{ path, method }` 객체와 문자열 경로 두 형태 모두 사용할 수 있다.
...
> **주의:** `exclude()`는 내부적으로 `path-to-regexp` 패키지를 사용한다. 동적 파라미터(`:id` 등)를 포함한 경로는 `posts/(:id)` 형태로 소괄호로 감싸야 올바르게 인식된다.
...
## 9. 서드파티 Express 미들웨어 사용하기

NestJS는 Express 위에서 동작하므로, npm에 공개된 **Express 미들웨어 패키지를 그대로 `apply()`에 전달**할 수 있다. 별도의 래핑 없이 바로 사용할 수 있다는 점이 큰 장점이다.

### 예시: helmet (보안 헤더 설정)

`helmet`은 HTTP 응답 헤더를 자동으로 설정하여 일반적인 웹 취약점을 방어하는 미들웨어다.
...
### 예시: morgan (HTTP 요청 로거)

`morgan`은 Node.js 생태계에서 널리 쓰이는 HTTP 요청 로깅 미들웨어다. NestJS의 `configure()` 안에서도 사용할 수 있다.

```typescript
// 설치: npm install morgan @types/morgan
// src/app.module.ts
import { Module, NestModule, MiddlewareConsumer } from '@nestjs/common';
...
`apply()`는 여러 미들웨어를 순서대로 받으므로, NestJS 클래스 미들웨어와 서드파티 미들웨어를 함께 체이닝할 수도 있다.
...
> **참고:** `app.use()`는 함수형 미들웨어(또는 서드파티 미들웨어 함수)만 받는다. 클래스 미들웨어를 전역 적용하려면 `configure()` 안에서 `forRoutes('*')`를 사용해야 한다.
