...
> **팁:** 실무에서 가장 많이 쓰는 파이프는 `ValidationPipe`(DTO 검증)와 `ParseUUIDPipe`/`ParseIntPipe`(파라미터 변환 및 검증)이다. UUID를 엔티티 ID로 사용한다면 `ParseUUIDPipe`를, 페이지네이션 숫자 파라미터에는 `ParseIntPipe`를 활용한다.
...
// POST /posts
{
  ...
  "tags": [{ "name": "NestJS", "color": "#E0234E" }, { "name": "백엔드" }]
}
```
...
@Controller('posts')
export class PostsController {
  // ParseIntPipe: 경로 파라미터 문자열을 정수로 변환
  @Get(':id')
  findOne(@Param('id', ParseIntPipe) id: number) {
    // GET /posts/1    -> id = 1 (number)
    // GET /posts/abc  -> 400 Bad Request
    return `게시글 ${id}`;
  }

  // ParseIntPipe: 숫자 파라미터 변환 (에러 상태 코드 커스터마이징)
  @Get('pages/:page')
  findPage(
    @Param(
      'page',
    ...
    // GET /posts/pages/abc -> 406 Not Acceptable
    return `페이지 #${page}`;
  }

  // DefaultValuePipe와 ParseIntPipe 조합 (숫자 쿼리 파라미터)
  @Get()
  findAll(
...
{
  "statusCode": 400,
  "message": ["제목은 필수입니다.", "내용은 최소 10자 이상이어야 합니다."],
  "error": "Bad Request"
}
```
...
export class UsersController {
  ...
  @Get(':id')
  findOne(@Param('id', ParseIntPipe) id: number) {
    // ParseIntPipe가 정수인지 검증 및 변환
    // 숫자가 아닌 값이 오면 자동으로 400 에러
...
{
  "statusCode": 400,
  "message": ["page는 1 이상이어야 합니다.", "limit는 100 이하여야 합니다."],
  "error": "Bad Request"
}
```
...
│   ├── posts.service.ts
│   └── dto/
│       ├── create-post.dto.ts       ← title, content, authorId(number) 검증
...
└── comments/
    ...
    ├── comments.service.ts
    └── dto/
        └── create-comment.dto.ts    ← content, authorId(number) 검증
```
...
| 내장 파이프         | `ValidationPipe`, `ParseIntPipe`, `ParseUUIDPipe`, `DefaultValuePipe` 등            |
| `ParseIntPipe`      | 문자열을 정수로 변환, 페이지네이션 `page` · `limit` 같은 숫자 쿼리 파라미터에 유용  |
| `ParseUUIDPipe`     | UUID 형식 검증, `version` 옵션으로 v1/v3/v4/v5 지정 가능, 엔티티 ID에 권장         |
...
| class-validator     | 데코레이터 기반 유효성 검사 (`@IsString()`, `@IsEmail()`, `@IsInt()` 등)            |
