...
4. [1-4. 커스텀 예외 만들기](#1-4-커스텀-예외-만들기)
5. [1-5. @Catch() 데코레이터와 ExceptionFilter 인터페이스](#1-5-catch-데코레이터와-exceptionfilter-인터페이스)
6. [1-6. ArgumentsHost 활용](#1-6-argumentshost-활용)
7. [1-7. 필터 바인딩 레벨](#1-7-필터-바인딩-레벨)
...
#### HttpException의 cause 옵션

`HttpException`을 던질 때 `cause` 옵션으로 원인 에러를 체이닝할 수 있다. 응답에는 포함되지 않지만 로깅 시 근본 원인 추적에 유용하다.

```typescript
// src/posts/posts.service.ts
try {
  await this.externalApi.fetchData();
} catch (error) {
  throw new InternalServerErrorException('외부 API 호출에 실패했습니다', {
    cause: error, // 원본 에러를 체이닝
  });
}
```

Exception Filter 안에서 `exception.cause`로 원인 에러에 접근할 수 있다.
...
### 1-8. BaseExceptionFilter 상속

완전히 새로운 에러 응답을 만드는 것이 아니라, **NestJS 기본 동작을 유지하면서 일부만 확장**하고 싶을 때는 `BaseExceptionFilter`를 상속한다.
...
@Catch() // 인자 없으면 모든 예외를 잡는다
export class AllExceptionsFilter extends BaseExceptionFilter {
  catch(exception: unknown, host: ArgumentsHost) {
    // 추가 작업 (로깅 등)
    console.error('예외 발생:', exception);

    // NestJS 기본 예외 처리 동작에 위임
    super.catch(exception, host);
  }
}
```

`BaseExceptionFilter`는 `HttpAdapterHost`를 생성자에서 주입받아야 하므로, `main.ts`에서 등록할 때 주의가 필요하다.
...
> **참고:** 완전히 커스텀한 에러 포맷이 필요하다면 `ExceptionFilter`를 직접 구현하는 방식(이 챕터의 주요 내용)을 사용한다. 기본 동작에 로깅만 덧붙이는 수준이라면 `BaseExceptionFilter` 상속이 더 간단하다.
