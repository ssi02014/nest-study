...
**플러그인 미적용 (수동으로 `@ApiProperty` 선언 필요)**:
...
**플러그인 적용 후 (`@ApiProperty` 없이 동일한 Swagger 문서 생성)**:
...
`@Controller` 데코레이터에 `version` 옵션을 추가하거나, `@Version()` 데코레이터를 메서드에 붙인다.
...
> **Tip**: 실무에서는 버전을 너무 많이 만들지 않는 것이 좋다. 새 버전이 늘어날수록 유지보수 비용이 커진다. 가능하면 하위 호환성을 유지하되, 파괴적 변경(breaking change)이 불가피할 때만 새 버전을 생성한다.
...
| Swagger CLI 플러그인 | `nest-cli.json`에 설정하여 `@ApiProperty` 없이도 DTO 스키마를 자동 생성 |
